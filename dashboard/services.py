...
def get_order_by_user(id):
    with closing(connection.cursor()) as cursor:
        ...
        food_order.payment_type,food_order.status,food_order.created_at FROM food_order 
...
def get_table():
    with closing(connection.cursor()) as cursor:
        ...
        GROUP BY food_orderproduct.product_id ,food_product.title ORDER BY count desc limit 10
        """)
